#!/usr/bin/env python3
"""
Threshold sweep: Convergence-aware fitness, convergence_threshold=0.20 (20%).
Baseline is 0.05 (5%), implemented as errors[i+1] < errors[i] * 0.95.

Fork of gp_leibniz_v3_minimal.py with ONLY these changes:
- Convergence bonus threshold: 0.95 → 0.80 (requires 20% error reduction)
- Output filenames use threshold_convaware_0.20 prefix
...
# 0.80 means 20% reduction required (baseline is 0.95 = 5% reduction)
CONV_THRESHOLD = 0.80  # CHANGED from 0.95 baseline
...
def compute_fitness(tree):
    ...
    # CHANGED: CONV_THRESHOLD=0.80 instead of 0.95 (20% reduction instead of 5%)
    n_improving = sum(
...
def write_evolution_json(seed_results):
    ...
    path = OUT_DIR / "threshold_convaware_0.20_data.json"
...
def write_convergence_csv(seed_results):
    ...
    if not rows:
        return
    path = OUT_DIR / "threshold_convaware_0.20_convergence.csv"
...
def write_results_txt(seed_results, unit_report, eval_report):
    ...
    header = "\n".join([
        "=" * W,
        "THRESHOLD SWEEP: CONVERGENCE-AWARE, THRESHOLD=0.20 (20%)",
        f"Minimal terminal set {{k, 1, -1, 2}}, no injection, 30-min budget",
        "=" * W,
        "",
        f"Threshold parameter: CONV_THRESHOLD = {CONV_THRESHOLD} (20% reduction required)",
        f"  Baseline: 0.95 (5% reduction required)",
        f"  Higher threshold is more selective — fewer consecutive pairs qualify",
    ...
    path = OUT_DIR / "threshold_convaware_0.20_results.txt"
    with open(path, "w") as f:
        f.write(header + "\n" + unit_report + "\n" + eval_report)
    print(f"  Wrote {path}", flush=True)
...
def main():
    global_t0 = time.time()
    print("=" * 72, flush=True)
    print(f"Threshold sweep: Conv-aware CONV_THRESHOLD={CONV_THRESHOLD} (20% reduction, baseline=0.95/5%)", flush=True)
