#!/usr/bin/env python3
"""
Parsimony Pressure Test: Fork of entropy_leibniz_v3_minimal.py.

Tests whether increasing LAMBDA_P (parsimony pressure) drives discovery
toward the canonical 9-node Leibniz form vs bloated equivalents.

Usage:
    python3 parsimony_test.py --lambda_p 0.01
    python3 parsimony_test.py --lambda_p 0.02
    python3 parsimony_test.py --lambda_p 0.05

Output files are named with the lambda value, e.g.:
    parsimony_lp0.01_results.txt
    parsimony_lp0.01_data.json
    parsimony_lp0.01_convergence.csv
"""
...
# ── Parse args first, before any fitness computation ──────────────────────────

parser = argparse.ArgumentParser(description="Parsimony pressure test for Leibniz GP")
parser.add_argument("--lambda_p", type=float, default=0.005,
                    help="Parsimony penalty coefficient (default: 0.005)")
...
W2 = 0.04
W3 = 0.03
LAMBDA_P = args.lambda_p  # Override with CLI argument
INFO_CAP = 50.0
INFO_NORM = 50.0
RATE_NORM = 5.0
MIN_GAIN = 0.5
...
# Output filename tag derived from lambda value
LP_TAG = f"lp{LAMBDA_P:g}"
...
def run_seed(seed_idx, seed_val, max_time, global_t0):
    ...
    while True:
        ...
        if gen % CHECKPOINT_INTERVAL == 0:
            elapsed = time.time() - t0
            with open(OUT_DIR / f"parsimony_{LP_TAG}_progress.json", "w") as f:
                json.dump({
...
def run_unit_tests():
    W = 72
    lines = ["=" * W, f"T — UNIT TESTS (Parsimony test, lambda_p={LAMBDA_P})", "=" * W]
    passed = failed = 0
    ...
    lines.append("\n  ── Parsimony check ──")
    lines.append(f"  LAMBDA_P = {LAMBDA_P}")
    lines.append(f"  LP_TAG   = {LP_TAG}")
    check("LAMBDA_P matches arg",
          abs(LAMBDA_P - args.lambda_p) < 1e-12,
          f"LAMBDA_P={LAMBDA_P}, arg={args.lambda_p}")
...
def run_vv_report(seed_results):
    ...
    lines.append(f"\nFitness under parsimony test (lambda_p={LAMBDA_P}):")
    ...
    # Node count stats for equivalent seeds
    equiv_nc = [r["best_ind"].node_count()
                for r in seed_results
                if r.get("analysis", {}).get("is_equivalent", False)]
    if equiv_nc:
        mean_nc = sum(equiv_nc) / len(equiv_nc)
        lines.append(f"  Mean node count (Leibniz seeds): {mean_nc:.1f}")
        lines.append(f"  Node counts: {equiv_nc}")
        canonical = sum(1 for nc in equiv_nc if nc == 9)
        lines.append(f"  Canonical 9-node form: {canonical}/{len(equiv_nc)}")
...
def write_entropy_json(seed_results):
    ...
    data = {
        ...
        "info_profiles": {
            "leibniz": lei_infos,
            "gp_v1": gp1_profile,
            "entropy_parsimony": entropy_profile,
        },
        "seeds": [
            {
                "seed": r["seed"],
                ...
                "node_count": r["best_ind"].node_count(),
                "elapsed": r["elapsed"],
        ...
        "convergence_curves": {
            ...
            "entropy_parsimony_best": best_curve,
        },
        "error_curves": {
            "leibniz": to_error_curve(lei_curve),
            "gp_v1_best": to_error_curve(gp1_curve),
            "entropy_parsimony_best": to_error_curve(best_curve),
        },
        "entropy_parsimony_expr": best_r["best_expr"],
        "gp_v1_expr": gp1.to_str(),
        "leibniz_expr": lei.to_str(),
    }

    path = OUT_DIR / f"parsimony_{LP_TAG}_data.json"
...
def write_convergence_csv(seed_results):
    ...
    if not rows:
        return
    path = OUT_DIR / f"parsimony_{LP_TAG}_convergence.csv"
...
def write_results_txt(seed_results, unit_report, eval_report, vv_report):
    ...
    header = "\n".join([
        "=" * W,
        f"PARSIMONY PRESSURE TEST: TEVV REPORT  (lambda_p={LAMBDA_P})",
...
def main():
    global_t0 = time.time()
    print("=" * 72, flush=True)
    print(f"Parsimony Pressure Test: lambda_p={LAMBDA_P}  tag={LP_TAG}", flush=True)
    ...
    prog = OUT_DIR / f"parsimony_{LP_TAG}_progress.json"
    if prog.exists():
        prog.unlink()
