#!/usr/bin/env python3
"""
Threshold sweep: Log-precision fitness, MIN_GAIN=1.0 (baseline is 0.5).

Fork of entropy_leibniz_v3_minimal.py with ONLY these changes:
- MIN_GAIN = 1.0 (instead of 0.5)
- Output filenames use threshold_mingain_1.0 prefix
...
INFO_NORM = 50.0
RATE_NORM = 5.0
MIN_GAIN = 1.0  # CHANGED from 0.5 baseline
...
def write_entropy_json(seed_results):
    ...
    path = OUT_DIR / "threshold_mingain_1.0_data.json"
...
def write_convergence_csv(seed_results):
    ...
    if not rows:
        return
    path = OUT_DIR / "threshold_mingain_1.0_convergence.csv"
...
def write_results_txt(seed_results, unit_report, eval_report, vv_report):
    ...
    header = "\n".join([
        "=" * W,
        "THRESHOLD SWEEP: LOG-PRECISION MIN_GAIN=1.0",
    ...
    path = OUT_DIR / "threshold_mingain_1.0_results.txt"
    with open(path, "w") as f:
        f.write(header + "\n" + unit_report + "\n" + eval_report + "\n" + vv_report)
    print(f"  Wrote {path}", flush=True)
