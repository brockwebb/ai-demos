#!/usr/bin/env python3
"""
Threshold sweep: Log-precision fitness, MIN_GAIN=0.1, t=6 terminals (boundary test).

Fork of entropy_leibniz_v3_minimal.py with ONLY these changes:
- MIN_GAIN = 0.1 (instead of 0.5)
- TERM_FIXED = ["k", 1, -1, 2, 3, -2]  (t=6, instead of t=4)
- MAX_SEED = 1800.0  (scaling grid budget, instead of 360s)
- MAX_TOTAL = 9000.0  (5 seeds × 1800s)
- Output filenames use threshold_mingain_0.1_t6 prefix

Tests whether lowering MIN_GAIN affects discovery at t=6, the boundary
where baseline (MIN_GAIN=0.5) achieves ~1/5 seeds (fragile).
"""
...
INFO_NORM = 50.0
RATE_NORM = 5.0
MIN_GAIN = 0.1  # CHANGED from 0.5 baseline
...
DIV_INJECT = 100
WORST = -1e9
MAX_TOTAL = 9000.0   # CHANGED: 5 seeds × 1800s
MAX_SEED = 1800.0    # CHANGED: scaling grid budget per seed
N_SEEDS = 5
...
TERM_FIXED = ["k", 1, -1, 2, 3, -2]  # CHANGED: t=6
EPHEMERALS = []
...
def write_entropy_json(seed_results):
    data = {
        "config": {
            ...
            "term_fixed": TERM_FIXED, "n_terminals": len(ALL_TERMINALS),
            "leibniz_injection": False, "min_gain": MIN_GAIN,
            "max_seed": MAX_SEED, "max_total": MAX_TOTAL,
        },
        "pi_over_4": PI_OVER_4,
        "seeds": [
            {
    ...
    path = OUT_DIR / "threshold_mingain_0.1_t6_data.json"
...
def write_convergence_csv(seed_results):
    ...
    if not rows:
        return
    path = OUT_DIR / "threshold_mingain_0.1_t6_convergence.csv"
...
def write_results_txt(seed_results, unit_report, eval_report, vv_report):
    ...
    header = "\n".join([
        "=" * W,
        "THRESHOLD SWEEP: LOG-PRECISION MIN_GAIN=0.1, t=6 TERMINALS (BOUNDARY TEST)",
        "=" * W,
        "",
        f"MIN_GAIN = {MIN_GAIN} (baseline = 0.5)",
        f"Terminal set: {TERM_FIXED}  (t=6, baseline is t=4)",
        f"pop_size={POP_SIZE}, n_seeds={N_SEEDS}, max_seed={MAX_SEED}s, max_total={MAX_TOTAL}s",
        "",
        "Purpose: Test whether lowering MIN_GAIN improves discovery at t=6,",
        "         where baseline (MIN_GAIN=0.5) achieves ~1/5 seeds.",
    ])
    path = OUT_DIR / "threshold_mingain_0.1_t6_results.txt"
    with open(path, "w") as f:
        f.write(header + "\n" + unit_report + "\n" + eval_report + "\n" + vv_report)
    print(f"  Wrote {path}", flush=True)
...
def main():
    global_t0 = time.time()
    print("=" * 72, flush=True)
    print(f"Threshold sweep: Log-precision MIN_GAIN={MIN_GAIN}, t=6 terminals (boundary test)", flush=True)
    print(f"  Terminals ({len(ALL_TERMINALS)}): {ALL_TERMINALS}", flush=True)
    print(f"  Budget: {MAX_SEED}s/seed, {MAX_TOTAL}s total", flush=True)
    print("=" * 72, flush=True)
    ...
    unit_report = f"Leibniz fitness={lei_fit:.6f}  GP-v1 fitness={gp1_fit:.6f}  Leibniz>GP-v1: {lei_fit > gp1_fit}"
    ...
    print(f"MIN_GAIN={MIN_GAIN}, t=6: {n_equiv}/{len(seed_results)} seeds found Leibniz", flush=True)
    print("=" * 72, flush=True)
