#!/usr/bin/env python3
"""
Threshold sweep: Log-precision fitness, MIN_GAIN=0.1 (baseline is 0.5).

Fork of entropy_leibniz_v3_minimal.py with ONLY these changes:
- MIN_GAIN = 0.1 (instead of 0.5)
- Output filenames use threshold_mingain_0.1 prefix
...
def write_entropy_json(seed_results):
    ...
    data = {
        ...
        "info_profiles": {
            "leibniz": lei_infos,
            "gp_v1": gp1_profile,
            "threshold_sweep": entropy_profile,
        },
        "seeds": [
            {
                "seed": r["seed"],
        ...
        "convergence_curves": {
            ...
            "threshold_sweep_best": best_curve,
        },
        "error_curves": {
            "leibniz": to_error_curve(lei_curve),
            "gp_v1_best": to_error_curve(gp1_curve),
            "threshold_sweep_best": to_error_curve(best_curve),
        },
    }

    path = OUT_DIR / "threshold_mingain_0.1_data.json"
...
def write_convergence_csv(seed_results):
    ...
    if not rows:
        return
    path = OUT_DIR / "threshold_mingain_0.1_convergence.csv"
...
def write_results_txt(seed_results, unit_report, eval_report, vv_report):
    ...
    header = "\n".join([
        "=" * W,
        "THRESHOLD SWEEP: LOG-PRECISION MIN_GAIN=0.1",
        ...
        f"  monotonicity = fraction of consecutive checkpoints with >= {MIN_GAIN} bit gain",
    ])
    path = OUT_DIR / "threshold_mingain_0.1_results.txt"
    with open(path, "w") as f:
        f.write(header + "\n" + unit_report + "\n" + eval_report + "\n" + vv_report)
    print(f"  Wrote {path}", flush=True)
...
def main():
    ...
    print(f"  MIN_GAIN = {MIN_GAIN}", flush=True)
    print("=" * 72, flush=True)

    print("\n[1/5] Unit tests ...", flush=True)
    # Inline minimal unit tests
    lei = make_leibniz_tree()
    gp1 = make_gp1_best_tree()
    ...
    print(f"  Seeds finding Leibniz: {n_equiv}/{len(seed_results)}", flush=True)
    for r in seed_results:
        ana = r.get("analysis", {})
